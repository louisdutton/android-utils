...
class BlockingDialog @Inject constructor(
    ...
    fun show(conversationIds: List<Long>, block: Boolean) = GlobalScope.launch {
        ...
        if (block) {
            markBlocked.execute(MarkBlocked.Params(conversationIds, BlockingClient.BUILT_IN_CLIENT_ID, null))
            blockingClient.block(addresses).subscribe()
        } else {
            markUnblocked.execute(conversationIds)
            blockingClient.unblock(addresses).subscribe()
        }
    }
